...
各実行は全156地点の `current_year.latest_date` がJSTの前日まで到達したか確認し、1地点でも未到達なら10分間隔で最大3回再取得します。それでも未到達の場合は、途中まで取得できた有効値を先にコミットしたうえでActionsを失敗にし、更新遅延を検知できる状態にします。既存値は取得失敗で消しません。
