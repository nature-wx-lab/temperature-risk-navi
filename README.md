...
各実行は全156地点の `current_year.latest_date` がJSTの前日まで到達したか確認します。気象庁ページに前日の行があり、最高・最低気温がともに明示的な欠測になっている地点だけが全地点の1%以内（現在は最大2地点）の場合は、有効値を更新したうえで警告付き成功にします。ページ未公開、取得失敗、または欠測・遅延がこの上限を超える場合は、10分間隔で最大3回再取得します。それでも未到達の場合は、途中まで取得できた有効値を先にコミットしたうえでActionsを失敗にし、ツールが正しく更新できない可能性をメール通知で検知できる状態にします。既存値は取得失敗や欠測で消しません。
